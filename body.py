...
def extract_body(path: str, max_frames: int = 12000,
    ...
    scale = 1.0
    ph = pw = 0
    # Une vidéo de téléphone est souvent filmée en 1080p/4K, contre 720p ou
    # moins pour un webcam ordinateur : la soustraction de fond et les
    # opérations morphologiques ci-dessous coûtent en O(pixels), donc à cette
    # résolution l'analyse peut prendre plusieurs dizaines de minutes au lieu
    # de quelques secondes. On les fait tourner sur une copie réduite ; le
    # suivi sous-pixel du torse (mesure d'équilibre) et la détection de
    # mouvement de caméra, eux, restent sur l'image pleine résolution.
    PROC_MAX_DIM = 960
    ...
    while idx < max_frames:
        ...
        if H == 0:
            H, W = frame.shape[:2]
            scale = min(1.0, PROC_MAX_DIM / float(max(H, W)))
            ph, pw = max(1, int(round(H * scale))), max(1, int(round(W * scale)))
            min_area = int(0.004 * ph * pw)
        ...
        fg = sub.apply(proc, learningRate=-1 if idx < warmup else 0.0006)
        ...
        if person is None:
            ...
            # Tout ce qui suit est mesuré sur `person`, à la résolution réduite
            # (`proc`) : on reconvertit chaque grandeur en pixels de l'image
            # d'origine (`inv`) pour que le reste du pipeline (calibration
            # px_per_m, prévisualisation, suivi du torse) ne voie aucune
            # différence avec l'ancien comportement pleine résolution.
            inv = 1.0 / scale
            ys, xs = np.nonzero(person)
            x0, x1, y0, y1 = int(xs.min()), int(xs.max()), int(ys.min()), int(ys.max())
            h = y1 - y0 + 1
            x0o, y0o, x1o, y1o, ho = x0 * inv, y0 * inv, x1 * inv, y1 * inv, h * inv
            cent.append((float(xs.mean()) * inv, float(ys.mean()) * inv))
            bbox.append((x0o, y0o, x1o - x0o + 1, ho))
            hgt.append(float(ho))
            ...
            haut = ys <= (y0 + 0.45 * h)
            trunk.append((float(ys[haut].mean()) if haut.any() else float(ys.mean())) * inv)
            band = ys >= (y1 - 0.25 * h)
            spread.append((float(xs[band].max() - xs[band].min() + 1) if band.any() else np.nan) * inv)
            area.append(float(person.sum() / 255.0) * inv * inv)
            foot.append(float(y1o))
            valid.append(True)
            if preview is None and idx > warmup + 5:
                vue = frame.copy()
                x0i, y0i, x1i, y1i = int(x0o), int(y0o), int(x1o), int(y1o)
                cv2.rectangle(vue, (x0i, y0i), (x1i, y1i), (40, 220, 60), 2)
                bande = int(y1o - 0.25 * ho)
                cv2.line(vue, (x0i, bande), (x1i, bande), (60, 160, 255), 1)
                cv2.putText(vue, "silhouette detectee", (x0i, max(16, y0i - 8)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (40, 220, 60), 1,
                            cv2.LINE_AA)
                preview = vue
            if torso_box is None and idx > warmup + 5 and ho > 0.2 * H:
                torso_box = (x0o, y0o, x1o - x0o + 1, ho)
        ...
        # Fait exprès sur `gray` (pleine résolution, pas `proc`) pour ne pas
        # dégrader cette précision sous-pixel.
        ...
        if prev_gray is not None and idx % 5 == 0:
            fg_full = cv2.resize(fg, (W, H), interpolation=cv2.INTER_NEAREST) \
                if scale < 1.0 else fg
            cam_moves.append(_camera_motion(prev_gray, gray, fg_full))
        prev_gray = gray
        idx += 1
